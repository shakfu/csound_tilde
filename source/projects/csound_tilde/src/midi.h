...
public:
	...
	int DequeueCompleteMessage(csbyte* buf, int n);
	...
	/* When m_sysex == true, any data bytes recieved through this function
       ...
       is set to false.
	   Locks/Unlocks m_lock. */
	void Enqueue(csbyte b, bool lock = true);

	/* Enqueue n bytes from b into m_buffer.  b should contain only 1 complete MIDI message.
	   Locks/Unlocks m_lock. */
	void EnqueueBuffer(csbyte* b, int n, bool lock = true);
...
private:
	...
	void DequeueBuffer(csbyte* b, int n);

	/* Dequeue a single byte from the buffer.
	   Does not lock/unlock m_lock.  Does not check m_buffer's size. */
	csbyte Dequeue();

	boost::circular_buffer<csbyte> m_buffer; // Contains the MIDI bytes.
